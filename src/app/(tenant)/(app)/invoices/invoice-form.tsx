...
export function InvoiceForm({
  ...
  const [taxRate, setTaxRate] = useState<number>(invoice ? Number(invoice.taxRate) : 0);
  const [shipping, setShipping] = useState<number>(invoice ? Number(invoice.shipping) : 0);
  const [status, setStatus] = useState<string>(invoice?.status || "DRAFT");

  const [showAdjustments, setShowAdjustments] = useState(
    Boolean(
      (invoice ? Number(invoice.discount) : 0) ||
        (invoice ? Number(invoice.taxRate) : 0) ||
        (invoice ? Number(invoice.shipping) : 0)
    )
  );
  ...
  const wrappedFormAction = (fd: FormData) => {
    ...
    fd.append("shipping", String(shipping));
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {showAdjustments && (
              <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700">Shipping</label>
                  <input
                    type="number"
                    step="0.01"
                    value={shipping}
