...
export default async function InvoicesPage({
  ...
  const unfilteredInvoices = await prisma.invoice.findMany({
    where: { deletedAt: null },
  ...
  const remainingDue = (i: (typeof unfilteredInvoices)[number]) => {
    const grand = computeInvoiceGrandTotal(i);
            ...
            <tbody className="divide-y divide-slate-100">
              {invoices.map((invoice: any) => (
                <tr key={invoice.id} className="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-900">${Number(invoice.amount).toLocaleString()}</div>
                  </td>
                  <td className="px-6 py-4">
